...
const Quadtree = (boundingBox, maxPointsPerNode) => {
    boundingBox;
    maxPointsPerNode;
    const points = [];    // array to host the points
    ...
    const insertPoint = point => {
        ...
        if(childNodes.length === 0){
            ...
            const ne = new Quadtree(new Rectangle({x:x+w,y:y+h,w:w,h:h}),2)
            const nw = new Quadtree(new Rectangle({x:x,y:y+h,w:w,h:h}),2)
            const se = new Quadtree(new Rectangle({x:x+w,y:y,w:w,h:h}),2)
            const sw = new Quadtree(new Rectangle({x:x,y:y,w:w,h:h}),2)
            childNodes.push(ne)
            childNodes.push(nw)
            childNodes.push(se)
            childNodes.push(sw);
